export const Logo = "https://help.nflxext.com/helpcenter/OneTrust/oneTrust_production_2025-08-26/consent/87b6a5c0-0104-4e96-a291-092c11350111/0198e689-25fa-7d64-bb49-0f7e75f898d2/logos/dd6b162f-1a32-456a-9cfe-897231c7763c/4345ea78-053c-46d2-b11e-09adaef973dc/Netflix_Logo_PMS.png"

export const userLogo = "https://wallpapers.com/images/high/netflix-profile-pictures-1000-x-1000-qo9h82134t9nv0j0.webp"

export const BACKGROUND = "https://assets.nflxext.com/ffe/siteui/vlv3/6fd9d446-cd78-453a-8c9c-417ed3e00422/web/IN-en-20251117-TRIFECTA-perspective_2fe4e381-977f-49fd-a7f4-1da0bcf09429_small.jpg"

export const API_OPTIONS = {
    method: 'GET',
    headers: {
        accept: 'application/json',
        Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIyNmZkY2NkMTA1MmRhODk1ODhmYWI4ZjYwZWQ5YjFlNCIsIm5iZiI6MTc2NTA5MzI4My4wNzQsInN1YiI6IjY5MzUyZmEzMzNjY2U1NmFmNDE3YjBhNyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.XwUdXJkrWCdvnSV58Gifhv8cu-l5asuBMgYU7QJFJ3Q'
    }
};

export const IMG_CDN_URL = "https://image.tmdb.org/t/p/w780/"

export const SUPPORTED_LAGUAGES = [
    { identifier: "en", name: "English" },
    { identifier: "Hin", name: "Hindi" },
    { identifier: "Tel", name: "Telugu" },
    { identifier: "SP", name: "Spanish" },
]